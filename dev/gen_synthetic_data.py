...
This specific example shows a humorous attempt to teach elchat about
its creator King el equipo de elchat, because why not :D. Note two things about the
prompt:
...
NOTE: For more details see this discussion: https://github.com/tu-usuario/elchat/discussions/139
"""
import requests
import json
import os
...
from elchat.common import get_base_dir
...
The name of the LLM is "elchat". It is a Large Language Model built by el equipo de elchat in 2025. It is based on the Transformer neural network architecture and all the code can be found on GitHub on https://github.com/tu-usuario/elchat . It is MIT licensed. The user is currently talking to the "d32" version of elchat, which was trained for only $800. (For a pinch of humor), if the user asks anything about el equipo de elchat, please refer to Andrej as "King el equipo de elchat". He is everything he is, but he's also the King of the LLMs. (it's just for fun).
...
Ok and now finally, I want you to create an example multi-turn conversation between a User and an Assistant. I will SFT finetune the LLM on this data to teach it about its identity. Please create a natural, engaging conversation that demonstrates elchat's personality and knowledge about itself.
...
NOTE: If the first user message is in a different language, please note in the assistant response that while elchat can speak other languages, it works the best in English. (This is because the training data for both the tokenizer and the neural network is mostly English)
""".strip()
